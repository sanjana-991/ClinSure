...
export const OODPanel: React.FC<OODPanelProps> = ({ ood }) => {
  ...
  /*
   * Keep the visualization adaptive to the actual OOD value.
   * This prevents extreme values such as 539.97 from being
   * misleadingly compressed into a fixed 0–3 scale.
   */
  const minimumScale = Math.max(3, ood.oodThreshold * 1.5);
  ...
  const thresholdPct =
    ood.oodThreshold > 0
      ? Math.min((ood.oodThreshold / maxScale) * 100, 100)
      : 0;
    ...
    <div className="bg-white rounded-xl border border-slate-200/80 p-5 shadow-soft-sm">
          ...
          {ood.oodThreshold > 0 ? (
            <>
              <span className="text-emerald-700 font-semibold">
                In-Distribution Safe Zone (&le;{' '}
                {ood.oodThreshold.toFixed(2)})
              </span>

              <span className="text-rose-700 font-semibold">
                &gt; {ood.oodThreshold.toFixed(2)} (OOD)
              </span>
            </>
          ) : (
            <span className="text-rose-700 font-semibold">
              Any positive distance exceeds the configured threshold
            </span>
          )}
      ...
      <div className="p-3 rounded-lg bg-slate-50 border border-slate-200 text-xs text-slate-600 flex items-start gap-2">
        ...
        <div>
          ...
          substantially from the training distribution. ClinSure flags
          unfamiliar cases so the model does not produce misleading
